...
extern "C"
{
	...
	float GetScaledParameter(ReverbController* item, Parameter2 param)
	{
		return item->GetScaledParameter(param);
	}

	void SetParameter(ReverbController* item, Parameter2 param, float value)
	{
	...
	void Process(ReverbController* item, float* inputL, float* inputR, float* outputL, float* outputR, int bufferSize)
	{
		item->Process(inputL, inputR, outputL, outputR, bufferSize);
	}
}
